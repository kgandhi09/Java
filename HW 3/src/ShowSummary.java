...
class ShowSummary {
	...
	public boolean equals(Object other) {
		...
		return this.equalsHelper(this.daytime, otherS.daytime) && this.equalsHelper(this.primetime, otherS.primetime)
	...
	private String toStringHelper(LinkedList<Show> theseShows) {
		...
		for (Show thisShow : theseShows) {
			output = output + "Title: " + thisShow.title + " | " + "Broadcast Time: " + thisShow.broadcastTime + "\n";

			for (Episode thisEp : thisShow.episodes) {
				output = output + "Episode Title: " + thisEp.epTitle + " | " + "Runtime: " + thisEp.runTime + "\n";
			}
